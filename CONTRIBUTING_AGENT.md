...
Project key prefix: ANTH.

Hierarchy: initiative -> epic -> task -> sub-task.

Non-hierarchical types: story.
...
- story can have parent initiative, epic, task.
...
- assignment -> Start research (Research), Revise idea (Idea)

- copywriting -> Publish (Published), Revise copy (Copywriting)

- editor_select -> Start copywriting (Copywriting), More research (Research)

- idea -> Charter assignment (Assignment)

- published -> Revise after publish (Copywriting)

- report -> Editor selection (Editor select), More research (Research)

- research -> Draft report (Report), Revise assignment (Assignment)
...
kanbus create "Fix crash on launch" --type story --priority 0 --parent <epic-id>

kanbus update <id> --status open --assignee "you@example.com"

kanbus update <id> --status blocked

kanbus comment <id> "Progress note"

kanbus list --status idea
