...
sequenceDiagram
    ...
    participant D as Software Director
    participant W as Coding agents

    R->>D: reports a bug
    D->>W: assigns separate work
    W->>D: returns code and evidence
    D-->>W: rejects an inadequate change
    W->>D: returns a corrected change
    D->>R: delivers the combined result
    R-->>D: checks the fix or sends it back
```
